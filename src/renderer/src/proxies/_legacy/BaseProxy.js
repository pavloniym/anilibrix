...
export default class BaseProxy {
    ...
    async submit(method, url, parameters = {}) {
        ...
        // Set headers
        // Add user-agent
        const axios = require('@/renderer/src/plugins/axios').default;
     ...
     */
    getApiEndpoint() {
        return require('@/renderer/src/store').default?.state?.app?.settings?.system?.connection?.host;
    }
    ...
    getRequestHeaders() {
        ...
        // Create headers
        const store = require('@/renderer/src/store').default;
        const headers = {};
